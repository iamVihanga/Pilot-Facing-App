...
const ProfileSettings = ({ data }) => {
    ...
    const [droneInsurance, setDroneInsurance] = useState(null)
        ...
        const getUserData = async () => {
            try {
                ...
                const { data, error } = await supabaseClient
                    .from('Employees')
                    .select('firstName, lastName, telNumber, flyerID, operatorID, confirmNoProof, proofDoc, droneInsurance, userSkills, userDrones')
                    .eq('email', user.email)
                ...
                if (data) {
                    ...
                    setMobile(userData.telNumber)
                    setFlyerID(userData.flyerID)
                    setOperatorID(userData.operatorID)
                    setConfirmNoProof(userData.confirmNoProof)
                    setProofDoc(userData.proofDoc)
                    setDroneInsurance(userData.droneInsurance)
                    setSkills(userData.userSkills)
                    setDrones(userData.userDrones)
                }
                setLoading(false)
            } catch (err) {
                console.log(err)
    ...
    const handleUpdate = async () => {
        try {
            ...
            if (typeof (proofDoc) === 'string') {
                ...
                if (data) {
                    proofDoc_link = `${process.env.NEXT_SUPABASE_STORAGE_BASEURL}/${data.path}`
            ...
            if (typeof (droneInsurance) === 'string') {
                ...
                if (data) {
                    insurance_link = `${process.env.NEXT_SUPABASE_STORAGE_BASEURL}/${data.path}`
    ...
    return (
        <div>
            ...
            <div className="bg-white rounded-xl w-full px-4 py-8 mt-6">
                ...
                {updateError && <p className='text-red-500 text-xs my-2'>{updateError}</p>}
                    ...
                    <div>
                        ...
                        <div className="flex flex-row gap-4 pt-4 pb-4">
                            <div>
                                ...
                                <input value={firstName} onChange={e => setFirstName(e.target.value)} type="text" className='sm:w-32 w-full mt-2 form-input px-3 py-3 text-xs' placeholder='Jaime' />
                            </div>
                            <div>
                                <p className="text-gray-400 sm:text-base text-sm">Last Name</p>
                                <input value={lastName} onChange={e => setLastName(e.target.value)} type="text" className='sm:w-32 w-full mt-2 form-input px-3 py-3 text-xs' placeholder='Harris' />
                            </div>
                            <div className='sm:block hidden'>
                                <p className="text-gray-400 sm:text-base text-sm">Mobile Number</p>
                                <input value={mobile} onChange={e => setMobile(e.target.value)} type="text" className='mt-2 form-input px-3 py-3 w-52 text-xs' placeholder='07840774043' />
                            </div>
                        </div>
                        <div className='sm:hidden block w-full '>
                            <p className="text-gray-400 text-sm">Mobile Number</p>
                            <input value={mobile} onChange={e => setMobile(e.target.value)} type="text" className='mt-2 form-input px-3 py-3 sm:w-52 w-full text-xs' placeholder='07840774043' />
                        </div>
                        ...
                        <div>
                            ...
                            <div className="flex gap-4">
                                <input value={flyerID} onChange={e => setFlyerID(e.target.value)} type="text" className="form-input sm:w-auto w-full px-3 py-3 text-xs" placeholder='Flyer ID' />
                                <input value={operatorID} onChange={e => setOperatorID(e.target.value)} type="text" className="form-input sm:w-auto w-full px-3 py-3 text-xs" placeholder='Operator ID' />
                            </div>
                            ...
                            <div className="mt-4 flex max-w-fit sm:flex-row flex-col items-center justify-between w-full h-12 gap-x-2 sm:gap-y-0 gap-y-2">
                                ...
                                } isChecked={confirmNoProof} setChecked={setConfirmNoProof} />
