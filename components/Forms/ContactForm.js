...
const ContactForm = () => {
    ...
    const validateData = () => {
        ...
        const errorStyle = 'w-full bg-primaryBlueLight text-[16px] text-primaryBlue h-12 rounded-md px-4 outline-none placeholder:text-red-400 border border-red-400'
        ...
        if (email === '') {
            ...
            validationError = true
            setError('Please fill all required fields !')
        } else if (!email.match(re)) {     // Validate is match to regex
    ...
    return (
        <div className='sm:mt-12 mt-8'>
            ...
            {error && <ErrorMessage error={error} setError={setError} className="mb-5" />}
            ...
            <div className="grid sm:grid-rows-3 grid-rows-5 sm:gap-y-7 gap-y-5">
                ...
                <div className='h-12 flex justify-between'>
                    <div className='flex justify-start '>
                        <button onClick={(e) => setTitle('Mr')} className={`${title !== 'Mr' ? 'bg-primaryBlueLight text-primaryBlue' : 'bg-primaryTealLight text-primaryTeal'} ${titleErr && 'border border-red-500 text-red-500'} title-button`}>Mr</button>
                        <button onClick={(e) => setTitle('Ms')} className={`sm:mx-5 mx-3 ${title !== 'Ms' ? 'bg-primaryBlueLight text-primaryBlue' : 'bg-primaryTealLight text-primaryTeal'} ${titleErr && 'border border-red-500 text-red-500'} title-button`}>Ms</button>
                        <button onClick={(e) => setTitle('Mrs')} className={`${title !== 'Mrs' ? 'bg-primaryBlueLight text-primaryBlue' : 'bg-primaryTealLight text-primaryTeal'} ${titleErr && 'border border-red-500 text-red-500'} title-button`}>Mrs</button>
                    </div>
                    ...
                    {!isMobile && <div className="flex-1 w-full">
                        <input
                            type="text"
                            className=' normal-input'
