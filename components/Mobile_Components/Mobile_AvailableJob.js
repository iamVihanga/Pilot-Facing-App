  ...
  return (
    ...
    >
      ...
      <Mobile_SidebarHeader
        ...
        centerComponent={
          <div className='flex flex-col items-center'>
            <p className="text-sm font-semibold">{currentJob.pilotExpertize}</p>
            <p className="text-xs text-gray-400">#{currentJob.id}</p>
            <button className={`px-2 py-1 mt-1 text-[.6rem] font-medium bg-green-100 text-green-500 rounded-md`}>{currentJob?.status}</button>
          </div>
        }
      />
      ...
      <div className='mt-32'>
        <div className="m-3 bg-white rounded-xl p-2">
          ...
          <p className=" text-gray-400 mt-5 font-normal text-base break-words">{currentJob.address}</p>

          <p className=" text-black mt-5 font-normal text-xl break-words"><span className='font-semibold'>Start Date:</span> {new Date(currentJob.date).toDateString()}</p>

          <p className=" text-black mt-5 font-semibold text-lg break-words">Customers Notes:</p>
          <p className=" text-black mt-2 font-normal text-sm break-words">{currentJob.customerNote}</p>


          <p className=" text-black mt-6 font-semibold text-lg break-words">Customers Details:</p>
          <div className={`${currentJob.status === 'Available' && 'blur-sm'}`}>
            <p className="text-black mt-3 font-normal text-sm"><span className="font-semibold">Name: </span>{currentJob.customerID.firstName} {activeJob.customerID.lastName}</p>
            <p className="text-black mt-3 font-normal text-sm"><span className="font-semibold">Email Address: </span>{currentJob.customerID.email}</p>
            <p className="text-black mt-3 font-normal text-sm"><span className="font-semibold">Phone Number: </span>{currentJob.customerID.phoneNumber}</p>
            <p className="text-black mt-3 font-normal text-sm"><span className="font-semibold">Company: </span>{currentJob.customerID.companyName}</p>
          </div>
